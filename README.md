...
- **Packages** — Brave Origin, AMD VA-API/Vulkan drivers (`mesa-dri-drivers`, `mesa-va-drivers-freeworld`, `mesa-vulkan-drivers-freeworld`, `libva-utils`), mpv, GNOME Boxes, VS Code, Google Cloud CLI, LibreOffice, dev tools, Flatpak, archives and fonts; stock Firefox is left installed.
...
- **Desktop & fonts** — GNOME dark theme, volume overamplification enabled (up to 150%), three window buttons, two-finger touchpad scrolling, GNOME Software autostart/search disabled, Fira Code Nerd Font and MS core fonts.
